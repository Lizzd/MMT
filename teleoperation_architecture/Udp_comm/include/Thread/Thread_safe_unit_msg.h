...
public:
    Thread_safe_unit_msg()=default;

    Thread_safe_unit_msg(Thread_safe_unit_msg const & other)
    {
    ...
    void wait_and_pop_front(std::string& value)
    {
    ...
    std::shared_ptr<std::string> wait_and_pop_front()
    {
        std::unique_lock<std::mutex> lk(mux);

        data_cond.wait(lk,[this]{return this->unused_data;});
        std::shared_ptr<std::string> res(std::make_shared<std::string>(data));
        unused_data=false;
        return res;
    }

    bool try_pop_front(std::string& value)
    {
        std::lock_guard<std::mutex> lk(mux);
