...
// -------------------------------------------------------------------------- //
// TensorBlockIO copies data from `main` tensor block, to the `dst` tensor block.
// It's possible to specify main->dst dimension mapping for the copy operation.
// Dimensions of `dst` specify how many elements have to be copied, for the
// `main` we need to know only stride to navigate through source memory buffer.
...
class TensorBlockIO {
 ...
 public:
  ...
  // Copies data to `dst` from `main`, using provided dimensions mapping:
  //
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE IndexType Copy(
    ...
    // Copy single scalar value from `main` to `dst`.
    if (NumDims == 0) {
      *(dst.data + dst.offset) = *(src.data + src.offset);
      return 1;
    }

    // Both `dst` and `main` must have contiguous innermost dimension. We also
    ...
    // the main. If dimensions are reordered, we might end up reading data from
    // the main with `stride != 1`.
    //
    ...
    // If all dimensions are of size 1, just copy a scalar from `main` to `dst`.
    ...
    // Dimension in the main that corresponds to the dst innermost dimension.
    ...
    // `main` memory, so we can do less linear copy calls.
    ...
    // Setup strides to read data from `main` and write to `dst`.
    ...
    // Iterate copying data from main to dst.
...
#undef COPY_INNER_DIM
  ...
  // Copy from `main` to `dst` with an identity main->dst dimension map. Returns
...
// `target->main` dimension map (see definition above).
//
...
// where `main` is a tensor expression. Explore if it is possible to rewrite IO
