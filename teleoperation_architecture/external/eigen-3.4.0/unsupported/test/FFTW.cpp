...
void test_complex2d()
{
    ...
    FFT<T> fft;
    Eigen::Matrix<Complex,nrows,ncols> main,src2,dst,dst2;

    main = Eigen::Matrix<Complex,nrows,ncols>::Random();
    //main =  Eigen::Matrix<Complex,nrows,ncols>::Identity();

    for (int k=0;k<ncols;k++) {
        Eigen::Matrix<Complex,nrows,1> tmpOut;
        fft.fwd( tmpOut,main.col(k) );
        dst2.col(k) = tmpOut;
    }
    ...
    fft.fwd2(dst.data(),main.data(),ncols,nrows);
    fft.inv2(src2.data(),dst.data(),ncols,nrows);
    VERIFY( (main-src2).norm() < test_precision<T>() );
